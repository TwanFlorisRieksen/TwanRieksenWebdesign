(() => {
  ...
  function bindSimple(root, ctx){
    ...
    // data-bind-class -> sets className using data-class-base + value
    $$('[data-bind-class]', root).forEach(el=>{
      const p = el.getAttribute('data-bind-class');
      const v = getByPath(ctx, p);
      if(v !== undefined){
        const base = el.getAttribute('data-class-base') || '';
        const val = String(v).trim();
        el.className = (base ? (base + (val ? (' ' + val) : '')) : (val || el.className));
      }
    });
...
function mountWerkwijze(page){
  ...
  if(ctaButtonsHost){
    ...
    buttons.forEach((b) => {
      ...
      a.href = b.href || '#';
      a.textContent = b.label || ((window.RW_CONTENT&&window.RW_CONTENT.site&&window.RW_CONTENT.site.ui&&window.RW_CONTENT.site.ui.default_cta_label)||'Open');
  ...
  function mountProjecten(page){
    ...
    const site = (window.RW_CONTENT && window.RW_CONTENT.site) ? window.RW_CONTENT.site : {};
    const ui = site.ui || {};
    ...
    (page.projects||[]).forEach(p=>{
      ...
      const ctxParts = [];
      const sep = (labels.separator !== undefined) ? String(labels.separator) : ' • ';
      const addPart = (labelKey, valueKey) => {
        const val = p[valueKey];
        const lab = labels[labelKey];
        if(val && lab){
          ctxParts.push(`${lab}: ${val}`);
        }
      };
      addPart('goal','goal');
      addPart('built','built');
      addPart('audience','audience');

      card.querySelector('.project-context').textContent = ctxParts.join(sep);
      ...
      a.href = p.href || '#';
      a.textContent = p.link_label || ui.default_small_link_label || 'Bekijk';
      if(p.external){
      ...
      // label for measurement (no GA changes)
      if(p.event_label){
        a.setAttribute('aria-label', String(p.event_label));
        a.setAttribute('data-cta-label', String(p.event_label));
      }
...
function mountLegal(page){
  ...
  async function init(){
    ...
    // expose content for other scripts (e.g., contact-form)
    window.RW_CONTENT = ctx;
    ...
    // Also ensure header CTA label updates if site.cta.primary_label differs
    const cta = $('.header-cta');
    if(cta){
      cta.textContent = site.cta?.primary_label || 'Offerte aanvragen';
      cta.href = site.cta?.primary_href || 'contact.html';
    }
  }
